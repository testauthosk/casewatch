...
def bot_numbers():
    ...
    try:
        return {
            ...
            # платных подписок бота за всё время (только реальные оплаты; без выданных руками и рефералок)
            "subsPaidAll": one("SELECT COUNT(DISTINCT tg_id) FROM subs WHERE source IN ('stripe', 'tribute')"),
